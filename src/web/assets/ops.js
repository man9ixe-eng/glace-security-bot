...
const state = { data: null, csrf: null, activeTab: 'dashboard', decision: null, promotionMode: 'create' };
...
function navAllowed(button) {
  ...
  return !capability || Boolean(state.data?.capabilities?.[capability]);
}
...
function showTab(tab) {
  ...
  $('#pageTitle').textContent = active?.dataset.title || active?.textContent?.trim() || 'Operations Overview';
...
function renderViewer() {
  ...
  $('#profileRole').textContent = viewer.tierLabel;
  $('#opsBadge').textContent = viewer.opsLabel;
  $('#opsBadge').className = `role-chip ${roleClass(viewer.tier)}`;
  ...
  $$('[data-capability]').forEach((node) => {
    ...
    node.hidden = !state.data.capabilities[capability];
  });
}
...
function renderDashboard() {
  ...
  const stats = [
    ['Promotion Submissions', d.capabilities.viewPromotions ? pendingPromotions : '—', d.capabilities.viewPromotions ? `${awaitingCompletion} awaiting completion` : 'Corporate access', 'role-intern', '♛'],
    ['Staff Actions', d.capabilities.viewStaffCases ? openCases : '—', d.capabilities.viewStaffCases ? `${d.cases.filter((x) => x.status === 'pending_approval').length} awaiting Board` : 'Senior Management access', 'role-management', '◇'],
    ['Current LOAs', d.loas.length, `${d.loaHistory.length} recent ended records`, 'role-senior', '◔'],
    ['Watch Records', d.capabilities.viewWatchRecords ? activeWatch : '—', d.capabilities.viewWatchRecords ? `${d.watchRecords.filter((x) => x.status === 'escalated').length} escalated` : 'Corporate access', 'role-corporate', '◉'],
    ['Restricted Records', d.capabilities.viewRestrictedRecords ? d.restrictedRecords.filter((x) => x.status !== 'archived').length : '—', d.capabilities.viewRestrictedRecords ? 'Board-level confidentiality' : 'Corporate Board access', 'role-board', '◆'],
    ['Documents', d.capabilities.viewDocuments ? d.documents.length : '—', `${d.posts.length} schedule/update posts`, 'role-presidential', '▤'],
  ];
  ...
  const approvals = [
    ...d.promotions.filter((x) => ['board_review', 'presidential_review', 'approved_awaiting_completion'].includes(x.status)).map((x) => ({
      title: `${x.candidateUsername || x.candidateId}: ${x.currentRank} → ${x.proposedRank}`,
      meta: `${x.submissionNumber} • ${prettify(x.status)}`,
      tier: x.proposedTier,
      tab: 'promotions',
    })),
    ...d.cases.filter((x) => x.status === 'pending_approval').map((x) => ({
  ...
  ].map(([tier, label]) => `<div class="rank-row ${roleClass(tier)}"><span class="rank-dot"></span><strong>${label}</strong><span>Tier ${tier}</span></div>`).join('');
  ...
  ].filter(([, cap]) => d.capabilities[cap]).map(([tab,, icon,label]) => `<button class="quick-action" data-go="${tab}"><b>${icon}</b>${label}</button>`).join('') || '<div class="empty-state"><strong>Your access is ready</strong>Use the navigation to view the tools assigned to your rank.</div>';
}
...
function promotionActions(item) {
  const c = state.data.capabilities;
  const viewer = state.data.viewer;
  const actions = [];
...
function renderCases() {
  ...
  $('#caseList').innerHTML = state.data.cases.length ? state.data.cases.map((item) => {
    const actions = [];
    if (c.approveSeriousCase && item.status === 'pending_approval') {
    ...
    if (c.approveSeriousCase && item.status === 'approved') {
...
function renderAll() {
  renderViewer();
  renderDashboard();
  if (state.data.capabilities.viewPromotions) renderPromotions();
  if (state.data.capabilities.viewStaffCases) renderCases();
  if (state.data.capabilities.viewWatchRecords) renderWatch();
  if (state.data.capabilities.viewRestrictedRecords) renderRestricted();
  if (state.data.capabilities.viewDocuments) renderDocuments();
  if (state.data.capabilities.viewPosts) renderPosts();
  if (state.data.capabilities.viewLoas) renderLoas();
  if (state.data.capabilities.viewAudit) renderAudit();
